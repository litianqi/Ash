  ...
  public:
    ...
    void render() override
    {
        ...
        const UniformsPerFrame per_frame = {
            .proj = glm::perspectiveLH(fov, aspect_ratio, 0.1f, 500.0f),
        ...
        for (uint32_t i = 0; i != kNumCubes; i++)
        {
            ...
            const vec3 offset = vec3(-1.5f * sqrt(kNumCubes) + 4.0f * (i % cubes_in_line),
                                     -1.5f * sqrt(kNumCubes) + 4.0f * (i / cubes_in_line), 0);
            const auto time = static_cast<float>(static_cast<double>(SDL_GetTicks()) / 1000.0);
            per_object[i].model = glm::rotate(glm::translate(mat4(1.0f), offset), direction * time, axes[i]);
        }
...
ASH_CREATE_APPLICATION(ash::CubeApp)
